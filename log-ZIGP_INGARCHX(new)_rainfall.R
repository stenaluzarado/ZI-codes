...
liko<-function(nob,yt,xt1,alpha0,alpha1,beta,psi,rho,om1,d1,d0){
  liko=0
  nu=numeric(nob)
  nu_s=numeric(nob)
  lam_s=numeric(nob)
  ...
  { 
    ...
    nu_s[t] = ((1-psi)/(1-rho))*nu[t]+om1*xt1[t-d1]
    lam_s[t]=exp(nu_s[t])
...
for (isi in 1:nosim)
{
  ...
  for (i in 1:M){
    ...
    lik1<-liko(nob,yt,xt1,alpha0,alpha1,beta,psi,rho,om1,d1,d0)
    lik2<-liko(nob,yt,xt1,star[1],star[2],star[3],psi,rho,om1,d1,d0)
    ...
    lik3<-liko(nob,yt,xt1,alpha0,alpha1,beta,psi,rho,om1,d1,d0)
    lik4<-liko(nob,yt,xt1,alpha0,alpha1,beta,psi_star,rho,om1,d1,d0)
    ...
    lik5<-liko(nob,yt,xt1,alpha0,alpha1,beta,psi,rho,om1,d1,d0)
    lik6<-liko(nob,yt,xt1,alpha0,alpha1,beta,psi,rho_star,om1,d1,d0)
    ...
    if(i  <= ind)
    {
      ...
      repeat
      {
        star1=rnorm(1,IK_mean3,IK_cov3)
        if ( (star1[1]>0)) {break}
        
        lognor6=dnorm(star1,IK_mean3,IK_cov3,log=T)
        lognor5=dnorm(old2,IK_mean3,IK_cov3,log=T)
      }
    
    lik7<-liko(nob,yt,xt1,alpha0,alpha1,beta,psi,rho,om1,d1,d0)+(d1-1)*log(om1)-d1*om1
    lik8<-liko(nob,yt,xt1,alpha0,alpha1,beta,psi,rho,star1[1],d1,d0)+(d1-1)*log(star1[1])-d1*star1[1]
    ...
    for(j in 1:d0){
      ...
      lik_b[j]=liko(nob,yt,xt1,alpha0,alpha1,beta,psi,rho,om1,j,d0)
    ...
    draws[i,] = c(alpha0,alpha1,beta,psi,rho,om1,d1)
    ...
    for (t in (d0+1):nob)
    {
      ...
      nu_star[t]= ((1-psi)/(1-rho))*nu_o[t]+om1*xt1[t-d1]
      lam_s[t]=exp(nu_star[t])
    ...
    if(i==ind)
    {
      ...
      IK_mean=c(mean(draws[1001:ind,1]),mean(draws[1001:ind,2]),mean(draws[1001:ind,3])) #for alpha0, alpha1, beta
      IK_cov=cov(draws[1001:ind,1:3])   #for alpha0, alpha1, beta
      IK_mean1=mean(draws[1001:ind,4])  #for psi
      IK_cov1=var(draws[1001:ind,4])    #for psi
      #IK_mean2=mean(draws[1001:ind,5])
      #IK_cov2=var(draws[1001:ind,5])
      IK_mean3=c(mean(draws[1001:ind,6])) #for om1 only
      IK_cov3=var(draws[1001:ind,6])  #for om1 only
  ...
  { 
    ...
    nu_star[t] = ((1-psi_mean)/(1-rho_mean))*nu_o[t]+om1_mean*xt1[t-d1_mode]
    lam_s[t]=exp(nu_star[t])
  ...
  ############################# PLOT
  names		 = c(list(expression(alpha0)),list(expression(alpha1)),list(expression(beta)), list(expression(psi)),list(expression(rho)),list(bquote(omega[1])),"d1")
  if (isi==1) 
  {
  ...
  #****************************************************************************
  #MCMCfile<-cbind(S1,S2,S3,S4,S5,S6,S7)
  ...
  ### save si2.out
  #write.csv(MCMCfile,"C:/Users/ACER/Downloads/ZI GROUP CODES/rainfall_old(tibanga).csv")
...
#logL_rain <- -0.5 * d[(ind+1):M, 1]  # Convert -2*logL to logL
#mlog_rain <- harmonic_mean_log(logL_rain)  # Save log marginal likelihood

# Show the result
#cat("Log Marginal Likelihood (Rain model):", mlog_rain, "\n")
...
plot = newdata %>% 
  ...
  xlab("") +
  ylab("") +
  ggtitle("Data 2: Suarez - Rainfall") +
  #ggtitle("Data 3: Buruun - Rainfall") +

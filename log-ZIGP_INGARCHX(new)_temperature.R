...
#logL_temp <- -0.5 * d[(ind+1):M, 1]  # Convert -2*logL to logL
#mlog_temp <- harmonic_mean_log(logL_temp)  # Save log marginal likelihood

# Show the result
#cat("Log Marginal Likelihood (Temp model):", mlog_temp, "\n")
#--------------------------------------
...
plot = newdata %>% 
  ...
  xlab("") +
  ylab("") +
  ggtitle("Data 2: Suarez - Temperature") +

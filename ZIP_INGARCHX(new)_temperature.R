# zero inflated Poisson (ZIP) INGARCHX* Model with delay parameters - rainfall
...
liko <- function(nob, yt, xt1, alpha0, alpha1, beta, rho, omega1, d1, d0) {
  liko = 0
  lam <- numeric(nob)
  lam_s <- numeric(nob)
  ...
  for (t in (d0 + 1):nob) {
    ...
    if(yt[t]==0){
      ...
      liko= liko+log(1-rho)+log(lam_s[t])+(yt[t]-1)
    }
...
for (isi in 1:nosim)
{
  ...
  for (i in 1:M){
    ...
    lik1<-liko(nob, yt, xt1, alpha0, alpha1, beta, rho, omega1, d1, d0)
    lik2<-liko(nob,yt,xt1,star[1],star[2],star[3], rho, omega1, d1, d0)
    ...
    lik5<-liko(nob,yt,xt1,alpha0,alpha1,beta,rho,omega1,d1,d0)
    lik6<-liko(nob,yt,xt1,alpha0,alpha1,beta,rho_star,omega1,d1,d0)
    ...
    if(i  <= ind)
    {
      repeat 
      {
        star1	= omega1 + rnorm(1,0,1)*step_c
        if ( (star1>0)) {break}
      }
    }else
      repeat
    ...
    lik7<-liko(nob,yt,xt1,alpha0,alpha1,beta,rho,omega1,d1,d0)+(c_1-1)*log(omega1)- c_2*omega1
    lik8<-liko(nob,yt,xt1,alpha0,alpha1,beta,rho,star1,d1,d0)+(c_1-1)*log(star1) - c_2*star1
    ...
    for(j in 1:d0){
      ...
      lik_b[j]=liko(nob,yt,xt1,alpha0,alpha1,beta,rho,omega1,j,d0)
    ...
    draws[i,] = c(alpha0, alpha1,beta,rho,omega1,d1)  
    ...
    for(t in (d0+1):nob)
    {
      ...
      if(yt[t]==0){
        ...
        lik9= lik9-2*log(1-rho)-2*log(lam_s[t])-2*(yt[t]-1)+2*(lam_s[t])+2*lgamma(yt[t]+1)
      }
    }
    ...
    if(i==ind)
    {
      ...
      IK_mean1=mean(draws[1001:ind,5])
      IK_cov1=var(draws[1001:ind,5])    #omega1
  ...
  d1_mode =getmode(draws[MCMC,6])
  
  
  for(t in (d0+1):nob)
  {
    lam[t] = alpha0_mean + alpha1_mean * yt[t - 1] + beta_mean * lam[t - 1]
    lam_s[t] = lam[t] + omega1_mean * xt1[t - d1_mode]
    
    if(yt[t]==0){
      lik10=lik10-2*log(rho_mean+(1-rho_mean)*exp(-(lam_s[t])))
    }
    else{
      lik10= lik10-2*log(1-rho_mean)-2*log(lam_s[t])-2*(yt[t]-1)+2*(lam_s[t])+2*lgamma(yt[t]+1)
    }
  }
  ...
  ############################# PLOT
  names		 = c(list(bquote(alpha[0])),list(bquote(alpha[1])),list(bquote(beta[1])), list(bquote(rho)),list(bquote(omega[1])), "d1")
  if (isi==1) 
  {
...
plot = newdata %>% 
  ...
  xlab("") +
  ylab("") +
  ggtitle("Data 1: Iligan - Temperature") +
